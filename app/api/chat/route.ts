...
export async function POST(req: NextRequest) {
  ...
  // Expect: { history: Array<{ role: 'user'|'ai', content: string }>, userMessage: string }
  const { history = [], userMessage = "" } = await req.json();

  // Build AI SDK messages (system + mapped history + current user)
  ...
  // Return a clean text stream (no SSE parsing needed on client)
  ...
  return new Response(stream, {
    headers: {
      ...
      // Optional: enable streaming in edge/network intermediaries
