...
import { Calendar, Clock, User, Video, MessageSquare, Headphones, ChevronLeft, ChevronRight } from 'lucide-react';
import { addDays, eachDayOfInterval, endOfWeek, format, isFuture, isSameDay, isToday, startOfWeek } from 'date-fns';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">Your upcoming and past mentoring sessions</p>
        </div>
      </div>
                    ...
                    {daySessions.map((bookingSession) => {
                      ...
                      return (
                        <div key={bookingSession.id} className="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-gray-900">
                          ...
                          {bookingSession.mentorName && (
                            <div className="mb-1 flex items-center gap-1">
                              <Avatar className="h-4 w-4">
                                <AvatarImage src={bookingSession.mentorAvatar} />
                                <AvatarFallback className="text-xs">
                                  {bookingSession.mentorName?.charAt(0) || 'M'}
                                </AvatarFallback>
                              </Avatar>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-gray-900 dark:text-white">{new Set(sessions.map(s => s.mentorId)).size}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-gray-900 dark:text-white">
                  {sessions.filter(s => s.status === 'scheduled' && isFuture(new Date(s.scheduledAt))).length}
                </p>
              </div>
            </div>
          </CardContent>
